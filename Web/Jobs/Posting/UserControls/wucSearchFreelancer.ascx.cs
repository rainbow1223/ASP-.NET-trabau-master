...
public partial class Jobs_Posting_UserControls_wucSearchFreelancer : System.Web.UI.UserControl
{
    ...
    public void GetProposals()
    {
        try
        {
            ...
            DataSet ds_proposals = obj.GetJobSubmittedProposals(Int32.Parse(JobId), Int64.Parse(UserID));
            var proposals = ds_proposals.Tables[0].ToDynamic().Select(x => new
            {
                ProposalId = MiscFunctions.Base64EncodingMethod(EncyptSalt.EncryptString(x.ProposalId.ToString() + "-" + JobId.ToString(), Trabau_Keys.Job_Key)),
                x.UserId,
                x.Title,
                x.EmailId,
                x.Name,
